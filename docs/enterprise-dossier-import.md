...
当前首发研究内容采用“一家公司 = 一个项目 = 一个根级 Markdown 文件”。仓库中的五份报告位于 `docs/enterprise-research/`，不再按章节拆成文件夹。

## 导入行为

`pnpm enterprise:dossier-import` 需要服务器 `.env` 中的 `DATABASE_URL`，由发布人员在服务器执行。脚本会：

1. 将五个固定项目恢复为 `public/published`；
...
预期每个项目只有一个公开文件树节点，`kind=markdown`，名称对应报告文件。`source`、旧 Revision 和审计 Commit 仍保留在数据库中供 AI 检索和历史追溯，但不会作为文件树重复展示。
