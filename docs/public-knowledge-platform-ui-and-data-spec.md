...
首页和搜索结果默认显示“推荐”，但推荐不是随机排序，也不是由模型随意决定。排序字段和权重必须可解释，并在接口中记录 `sort`。

| 排序 | 规则 | 适用场景 |
| --- | --- | --- |
| 推荐 | 公开状态、证据完整度、近期活跃度、有效来源数和去重互动的确定性综合分 | 首次探索 |
| 最近更新 | 最新公开 Merge Commit 时间倒序 | 找近期变化 |
| 最近发布 | 首个公开版本发布时间倒序 | 找新项目 |
| 阅读最多 | 去重阅读人数倒序，阅读数相同时按更新时间 | 找覆盖面大的研究 |
| 最受认可 | 点赞数与“有帮助”反馈倒序，排除已删除用户 | 找高认可内容 |
| 讨论最多 | 未解决评论线程数和有效回复数倒序 | 找争议或活跃主题 |
| 贡献最多 | 已合并贡献者人数倒序 | 找协作项目 |
| 来源最完整 | 当前公开版本的有效去重来源数倒序 | 找证据丰富内容 |
| 待核验优先 | `needs_verification` / `conflict` 内容优先 | 做事实复核 |
| 我的关注 | 仅登录用户：关注作者或项目的最新更新 | 个人信息流 |

排序规则：

- 所有排序都只使用公开主版本和真实聚合统计；草稿、私有项目和被隐藏内容不参与。
- 互动量必须按用户或签名访客去重，并设置时间衰减，避免旧项目永远占据榜首。
- 推荐分不使用虚构热度，不以评论数量单独代表质量；证据缺失或冲突的内容要降低推荐权重。
- 接口返回 `sort`、`sort_version` 和排序时间，便于审计和复现。
- 字段缺失时使用明确的最后排序规则，不用随机数；统计尚未聚合时显示“统计更新中”。

### 11.2 筛选条件

基础筛选采用左侧栏，更多条件进入 shadcn `Sheet` 或 `Popover`。多个筛选条件之间使用 AND，同一条件的多个值使用 OR。

| 条件 | 示例 | 来源 |
| --- | --- | --- |
| 内容类型 | 项目、报告、文件、来源、作者、评论 | 索引类型 |
| 项目分类 | 企业、行业、政策、技术 | `knowledge_project.category` |
| 作者/贡献者 | 用户名或显示名 | `platform_profile`、`content_attribution` |
| 标签 | 跨境电商、SaaS、网络安全 | `project_tag` |
| 来源类型 | PDF、网页、图片、Excel、Markdown | `source.kind` / 文件 MIME |
| 证据状态 | 事实、推断、待核验、冲突 | `evidence_state` |
| 发布时间 | 今天、7 天、30 天、自定义范围 | `published_at` |
| 更新时间 | 最近 24 小时、7 天、自定义范围 | 最新公开 Merge Commit |
| 互动范围 | 阅读、点赞、评论、收藏区间 | `project_stats` |
| 协作状态 | 有待审核 MR、已合并贡献、无冲突 | `merge_request` / Diff |
| 文件能力 | 含 PDF、图片、表格、可全文检索 | Asset/索引状态 |
| 许可证 | CC BY、保留所有权利等 | `knowledge_project.license` |
| 关注关系 | 我关注的作者/项目 | 登录用户关系表 |

筛选交互必须满足：

- 当前筛选以可关闭的条件 Chip 显示，提供“一键清除全部”。
- 搜索词、排序、分页和筛选同步到 URL，可复制链接后复现同一结果。
- 结果为空时显示当前条件和“清除筛选”，不能只显示无意义的空白页。
- 筛选请求失败时保留用户已选条件，并提供重试；不能悄悄退回未经筛选的全量结果。
- 结果列表显示命中类型和命中字段，例如“标题命中”“正文命中”“作者命中”“来源命中”。
- 登录相关筛选在未登录时显示登录入口，但不把私有项目数量泄露给游客。

### 11.3 统一成功/失败反馈

所有会改变数据、触发请求、上传文件、切换版本或执行 AI 操作的动作必须有完整状态：

```text
idle -> pending -> succeeded
                 -> failed -> retryable / blocked
```
...
| 状态 | 反馈方式 | 示例 |
| --- | --- | --- |
| pending | 按钮禁用、Spinner、进度或 Skeleton | “正在保存草稿…” |
| succeeded | shadcn Toast + 就地状态更新 | “已保存到草稿分支” |
| failed | 就地 Alert + Toast；保留输入和重试按钮 | “保存失败，版本已变化，请重新加载” |
| blocked | 明确权限/规则原因，不提供假成功 | “公开主版本只读，请创建草稿” |
| degraded | 显示降级原因和影响范围 | “搜索服务暂时不可用，仅显示缓存结果” |

具体要求：

- 成功反馈必须包含对象和结果，例如“评论已发布”“已关注作者”“文件校验通过”。
- 失败反馈必须包含可读原因、错误码（内部）和下一步动作；网络错误提供重试，权限错误提供登录或返回入口。
- 关键操作同时在页面内保留状态，Toast 消失后用户仍能知道结果。
- 不用颜色作为唯一信号；黑白灰界面使用图标、文字和边框组合表达成功、警告和失败。
- 上传、解析、索引、AI 生成和合并操作必须显示阶段进度，刷新页面后从服务端状态恢复。
- 乐观 UI 只能用于可逆的点赞、关注和收藏；请求失败必须回滚本地状态。
- 删除、撤回、合并和下架使用 shadcn `AlertDialog` 二次确认，并写入审计事件。
- 任何接口返回 2xx 但业务状态不是成功时，前端不能按成功处理，必须读取领域状态。

### 11.4 shadcn/ui 组件映射

| 场景 | 组件 | 约束 |
| --- | --- | --- |
| 成功/失败短提示 | Toast | 自动消失但不承载唯一信息 |
| 阻断和表单错误 | Alert | 就地显示，保留用户输入 |
| 删除/合并确认 | AlertDialog | 显示目标、版本和不可逆后果 |
| 高级筛选 | Sheet / Popover / Checkbox / Select | 条件同步 URL |
| 排序 | Tabs 或 Select | 当前选项有明确选中态 |
| 贡献者列表 | Dialog / Sheet / ScrollArea | 支持搜索、分页和作者跳转 |
| 评论楼中楼 | Collapsible / ScrollArea | 展开状态稳定，移动端用 Sheet |
| 文件预览 | ResizablePanel / Tabs | 预览失败保留原文件信息 |
| 上传进度 | Progress | 显示阶段和百分比，不伪造完成 |
| 加载列表 | Skeleton | 尺寸稳定，避免布局跳动 |
