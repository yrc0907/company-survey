...
| 2026-09-02 | 香港 ECS `47.57.138.55` | 发布 `bcaca01` 后通过 SSH 隧道直连真实 App/数据库，`pnpm test:e2e:platform`：10/10 通过；包含全站搜索 API、段落锚点入口、讨论区加载/匿名登录门槛、作者主页、Star、移动端；OSS 隔离对象真实 Put/Head/Delete 通过。迁移 `017_comment_anchors.sql` 已应用。活动/身份迁移随后部署到 `20dce8c`，需在新版入口复跑 11/11；ESA 返回 ICP/525，443 公网验收待人工切换后复跑 |
