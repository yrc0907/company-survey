...
本次香港迁移使用分支提交 `778e371`，服务器目录为 `/srv/research-workbench`。以下为实际命令与运行结果，不包含任何密码、Key 或 Basic Auth 凭据：
...
| 数据库 | 容器内 `/api/healthz` 返回 `200` 和 `persistence: "postgres"`；旧服务器备份已恢复，`project-huice` 存在，迁移表包含 `008_asset_ingestion_worker.sql` |
...
应用 `3000` 和数据库 `5432` 均未向公网发布。解析 Worker 通过 `ingestion` profile 按需运行，未作为常驻服务启动。当前安全组仍需收尾：将 SSH `22` 从 `0.0.0.0/0` 限制为可信管理来源，并删除 Linux 实例不需要的 RDP `3389` 规则。
