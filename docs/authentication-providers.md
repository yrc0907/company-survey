...
邮箱/手机号验证码登录对已注册和未注册目标使用相同响应；未知目标只有在 OTP 正确后才自动创建最小账户并建立 Session。密码找回对未知邮箱仍返回统一响应，避免账号枚举；验证码 60 秒内不可重发，最多尝试 5 次，过期或消费后不能重放。
...
- 已验证手机号验证码登录不能创建重复用户；未绑定手机号在 OTP 成功后自动创建带内部占位邮箱的最小账户。
- 未注册邮箱或手机号的 OTP 登录成功后自动生成唯一用户名和随机密码哈希；密码不会返回客户端，用户可后续绑定邮箱或通过找回密码设置密码。
