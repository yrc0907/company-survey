...
迁移 `022_public_company_seed.sql` 为公开首页和个人 Research API 提供四个企业研究项目：
慧策、泛微网络、深信服、信锐科技。资料包只包含企业官网公开入口和一段保守的人工摘要，
...
- `knowledge_project` 保存公开项目标题、摘要、分类、标签、许可证和 `needs_verification` 状态；四个项目均为 `public/published`，方便首页读取。
...
契约 `lib/services/platform/public-seed.contract.ts` 读取迁移和本文档，检查四个项目、URL、
