...
历史迁移 `025_public_research_file_tree.sql` 曾建立研究目录和章节节点；当前完整研究导入采用每个项目一个根级 Markdown 文件，详见 [企业完整 Markdown 导入](enterprise-dossier-import.md)。来源、Revision 和审计记录仍可追溯，不写入虚假阅读、点赞、评论、关注或贡献行为。
