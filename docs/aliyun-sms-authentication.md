...
ALIYUN_SMS_SCHEME_CODE=<控制台方案编码>
ALIYUN_SMS_SIGN_NAME=<控制台签名名称>
ALIYUN_SMS_TEMPLATE_CODE=100001
```

Provider 使用 `Dypnsapi/2017-05-25` RPC 参数和 HMAC-SHA1 签名。验证码放在 `TemplateParam` 的 `code`/`min` 字段，挑战 UUID 作为 `OutId`，重试时复用同一请求参数。默认端点只允许 HTTPS 覆盖；缺少 RAM 凭据或方案编码时保持未配置，不伪造发送成功。
