...
- [~] 五家企业已有可回溯来源、数据字段和 Yu 判断；GraphRAG-lite 关系图 UI、财务指标卡与区间位置图已上线，完整历史时间序列图、产品/上下游扩展图、版本差异和自动研究质量检查仍待接入，缺失数据保持 `needs_verification`
...
- [~] 三家冻结上市企业已保留公开行情与财报数据、Yu 的趋势判断及指标卡/区间图；分产品收入/利润、完整年报、竞争对手定量对比和历史序列交互图仍待补齐，未上市主体标记为不适用/待核验；关系图 UI 已通过 `graph` Tab 上线
