...
- [ ] 接入阿里云企业邮箱真实发信：专用发件账号、SMTP/API 适配器、验证邮件、密码找回邮件、登录验证码邮件、发送超时/退信记录和模板版本；应用不得使用管理员密码，凭据仅放服务器 `.env`
- [ ] 在 ESA DNS 区核验邮箱 MX、SPF、DKIM、DMARC 与发件域名对齐；邮件主机记录保持 DNS 解析，不走 CDN 代理，并记录投递成功率与退信原因
- [ ] 接入阿里云短信真实 Provider：签名“恒创联众”、登录/注册模板 `100001`、验证码生成/哈希存储/过期/消费、供应商请求幂等、失败重试和余额/套餐告警；`AccessKey`/`appKey` 不进入代码、文档、镜像或日志
- [ ] 接入阿里云图形验证真实服务端校验：发送短信/邮件、登录、找回密码和绑定手机号前必须校验场景票据；票据过期、重复使用、校验超时和供应商不可用时 fail closed
- [ ] 增加邮箱/手机号身份字段与唯一约束、验证时间、绑定/换绑审计和账号合并规则；邮箱或手机号已属于其他用户时禁止静默创建重复账户
- [ ] 建立统一 `verification_challenge` 数据模型和 API：邮箱/短信共用状态机，记录用途、哈希、过期时间、尝试次数、消费时间、请求 IP/设备哈希和供应商消息 ID，不保存验证码明文
- [ ] 增加验证码与登录风控：单手机号/邮箱、IP、设备、账号多维限流，60 秒重发、错误次数上限、异常封禁、同响应防账号枚举；低流量 PostgreSQL 兜底，公开规模接入 Redis
...
- [~] 已新增迁移 `022_public_company_seed.sql` 和四个企业公开官网摘要；香港数据库执行迁移并复核官网快照后才能标记完成
- [x] 建立公开首发数据包清单：慧策、泛微网络、深信服、信锐科技分别记录项目 ID、来源 URL、摘要、SHA-256、来源类型、抓取时间和待核验边界
- [~] 四个首发项目已导入公开 URL/摘要、来源元数据和 `needs_verification` 状态；年报/公告、公开访谈、价格和竞品等独立证据仍待后续资料包补充
- [ ] 为首发项目生成完整文件树和章节正文（研究结论、公司/产品、市场与竞品、商业模式、客户与场景、政策、风险、证据目录、开放问题），正文引用必须能回跳到来源和段落
- [ ] 将企业、产品、行业、竞品、政策和关系边写入统一实体模型；同名公司、品牌、产品和集团关系需要人工确认后再合并
- [ ] 新增幂等数据导入脚本与 manifest，导入前校验来源可访问性、内容哈希、许可证和重复项目；生产数据库只接受通过核验的数据包
- [ ] 首页展示真实数据库项目和真实聚合统计；禁止用静态阅读数、Star、评论、贡献者或虚构用户填充空白，空数据必须显示明确空状态
