...
- [~] Auth.js 邮箱/密码注册、登录和 Session/RBAC 校验已实现；GitHub OAuth 可配置状态已公开，邮箱验证与找回密码仍待接入真实邮件 Provider
 - [~] 接入阿里云企业邮箱真实发信：SMTP 适配器、验证邮件/密码找回/登录验证码模板和超时边界已实现；香港 ECS 已写入运行时配置，但 `postmaster@webyrc.com` 测试返回 SMTP `526 Authentication failure`，需在企业邮箱控制台确认 SMTP 客户端服务/应用密码后重试，正式环境改用专用发件账号
