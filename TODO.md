...
- [ ] 建立持久化社区 seed batch：40-60 个跨城市/行业用户、完整 profile/头像、关注关系、收藏/Star、项目 owner、贡献者、维护者和审核者；所有记录写入正式 PostgreSQL 并可从页面互相跳转
- [ ] 为十二个企业项目写入可追溯社区活动：阅读、Star、关注、项目/段落评论与楼中楼回复、修改申请、Review、Merge、贡献归因、通知和 365 天活动热力图；所有统计由同一事件事实聚合，不写前端静态数字
- [ ] 建立 seed batch 重建/清理脚本、唯一约束校验和跨表一致性验收：用户、互动、附件、活动、统计、项目和引用必须可回溯，重复运行不得重复计数
