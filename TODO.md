...
逐项统计：`[x]` 已完成 50 项，`[~]` 部分完成 62 项，`[ ]` 未开始 0 项。这里的 0 项“未开始”不代表全部交付：62 项仍缺真实账号/Provider 回归、生产运维演练、完整 UI/API 闭环或规模化能力，不能改成完成来充数。交付判断、剩余动作和关闭顺序见 [项目交付状态与收尾清单](docs/project-delivery-status.md)。

当前公开首发范围仍冻结为五家公司：慧策掌上先机、泛微网络、深信服、信锐科技、牧原食品；认证仍按产品决策保持关闭。
...
- 场景用户模型要求覆盖不同城市、行业、研究方向和贡献角色；每个账号有独立 profile、头像、项目归属、关注关系和可跳转活动，不能全部归属同一个管理员账号。当前真实 active 账号不足时保持空状态，不用假账号填充。
...
1. **P0：安全与可恢复性。** 限制 SSH 来源、删除无用 RDP 规则、执行异机备份与恢复演练，并补磁盘/容器/证书告警。
2. **P1：真实内测闭环。** 在保持公众认证关闭的前提下，用内测账号完成认证、OSS 直传、解析、Diff/MR/审核/合并、通知深链和署名回放。
3. **P2：检索与记忆规模化。** 接入来源刷新路由/定时任务、视觉解析、pgvector ANN、真实指标采集、完整 Resume 和记忆清理。
4. **P3：五家公司研究深度。** 补独立年报/访谈/竞品证据和有来源的分产品定量字段，缺失信息继续标记 `needs_verification`。

## Multi-Agent 知识工作流（2026-09-05）

设计与边界见 [Multi-Agent Knowledge Workflow](docs/multi-agent-knowledge-workflow.md)。该能力面向整个平台，不限制为企业调研；Agent 只能通过受控工具和结构化 Task State 协作，不能绕过 Scope、Patch、Review 或 Merge。
...
- [~] AI 助手已改为登录后使用并由服务端校验 `401 AUTH_REQUIRED`；登录前占位和内测门槛已完成，Redis 共享限流、额度计费和跨实例滥用防护待认证开放后评估
...
- [~] AI 状态覆盖未配置、准备上下文、结果和错误；项目对应报告 ID 投影已修复发送前短路，真实工具调用时间线、流式输出、重试策略和新单 report.md 项目仍待实现
