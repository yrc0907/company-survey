...
- 公开环境不使用演示用户、虚构企业、虚构阅读量、虚构点赞、虚构评论、虚构关注或静态热度数字；所有用户、项目、来源、互动和统计都必须由真实注册、真实资料导入或真实用户行为产生。
- 禁止直接向生产数据库嵌入“模拟真实用户”的登录记录、密码、头像、阅读事件或社交关系；没有真实用户就显示空状态，不能用数字填满页面。
- 自动化 E2E 可以使用明确标记的隔离测试账号和测试数据，但必须使用独立数据库/项目 Scope，不能进入公开首页、作者主页、排行榜、热力图或项目统计。
- 需要丰富社区内容时，优先邀请真实研究者、行业从业者和企业用户完成注册、上传、评论、关注和贡献；每条公开互动保留用户、时间、目标和审计事件。
- 真实资料不足时保留 `needs_verification`、空统计或“暂无活动”，不得由 AI、脚本或管理员补造事实和热度。
...
- [ ] 将十二个首发项目全部扩展到完整企业研究档案：公司概况、产品矩阵、客户/场景、商业模式、价格、竞品、行业位置、AI/技术、政策契合、风险、时间线、图表、关系图和逐条来源；缺失字段保持空值或 `needs_verification`
...
- [ ] 建立真实用户增长与内容贡献流程：邀请/注册入口、邮箱或手机验证、个人资料、真实头像、上传研究、提交修改、评论回复、关注、Star 和通知；禁止通过数据库脚本制造公开互动
- [ ] 建立公开统计真实性校验：阅读、点赞、评论、关注、贡献者和热力图均从 append-only 事件聚合，支持明细抽样核对、异常行为标记和删除/下架后的重算
- [ ] 自动化登录/注册/上传/评论/MR/审核验收仅运行于隔离测试 Scope；测试账号、测试事件和测试对象不得被公开 API 查询或计入生产统计
