...
- [~] 清理除五家冻结企业外的项目、来源、文件树、企业关系和企业专属统计；执行前先做 PostgreSQL/OSS 成对备份，使用可回滚迁移，完成后运行公开首页、搜索、文件树和统计一致性验收
- [ ] 重新生成五家企业的独立研究包：执行摘要、产品矩阵、客户/场景、商业模式、市场、竞品、经营财务、技术/AI、生态、政策、风险、情景推演、反方证据、待核验问题和来源哈希
- [ ] 为五家企业补齐可点击证据链、数据表、趋势图、产品/上下游关系图、版本差异和研究质量检查；缺失数据保持 `needs_verification`
- [~] 登录暂不对公众开放：按钮和所有需要身份的入口显示“登录功能暂未开放，仅对内测用户开放”；短信、邮箱、OAuth 不触发，Auth.js 通过 `PUBLIC_AUTH_ENABLED` 开关保留待重启
...
- [~] 五家冻结项目已通过官方官网快照富化器追加来源、哈希、来源 Chunk、研究结论/产品/市场/商业模式/客户/政策/风险/证据目录和研究者分析章节；价格、财务、独立竞品、时间线、图表和关系图仍需逐家公司补充，缺失字段保持空值或 `needs_verification`
