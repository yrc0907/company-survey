...
- [x] 已执行迁移 `022_public_company_seed.sql`、`024_public_company_seed_additional.sql`、`025_public_research_file_tree.sql`、`029_muyuan_foods_seed.sql`；香港数据库已复核 13 个项目、项目文件树和研究章节
- [x] 建立公开首发数据包清单：慧策、泛微网络、深信服、信锐科技、有赞、纷享销客、金蝶、奇安信、安恒信息、启明星辰、钉钉、Lark，均记录项目 ID、来源 URL、摘要、SHA-256、来源类型、抓取时间和待核验边界
- [~] 十三个首发项目已具备公开 URL/摘要、来源元数据和 `needs_verification` 状态；年报/公告、公开访谈、价格、竞品与独立行业证据仍待后续资料包补充
- [~] 十三个项目已通过官方官网快照富化器追加来源、哈希、来源 Chunk、研究结论/产品/市场/商业模式/客户/政策/风险/证据目录和研究者分析章节；价格、财务、独立竞品、时间线、图表和关系图仍需继续导入真实资料，缺失字段保持空值或 `needs_verification`
- [~] 8 家上市企业已追加公开行情与财报快照及 Yu 的趋势判断；分产品收入/利润、完整年报、竞争对手定量对比、图表交互和关系图 UI 仍待补齐，未上市企业标记为不适用/待核验
- [~] 已通过迁移 `025_public_research_file_tree.sql` 和 `029_muyuan_foods_seed.sql` 为十三个项目建立研究文件夹/章节模板；章节正文已开始追加真实官网快照和 Yu 的分析，引用必须能回跳到来源和段落
...
- [x] 持久化社区 seed batch 已执行到香港 PostgreSQL：49 个跨城市/行业用户、profile、关注、Star、去重阅读、项目 owner/member、贡献归因和审核角色均已写入，重复运行不会重复计数
- [x] 十三个企业项目已写入可追溯社区活动：78 评论/回复、79 Star、147 关注、104 去重读者、832 日阅读、13 MR、13 Review、13 Merge、117 通知和 369 活动事件；统计由同一事实聚合
