...
export async function GET(_request: Request, context: { params: { branchId: string } }) {
  try { const actor = await getAuthenticatedActor(); const branch = await new CollaborationService(getCollaborationRepository(), getPlatformRepository()).getBranch(context.params.branchId, actor); return json({ branch }); }
