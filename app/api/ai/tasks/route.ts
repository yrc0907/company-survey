...
const taskSchema = z.object({ reportId: z.string().min(1).max(200), question: z.string().trim().min(1).max(1_000), selectedText: z.string().max(8_000).optional(), selectedSectionId: z.string().max(200).optional(), execution: z.enum(["run", "queue"]).optional() }).strict();
