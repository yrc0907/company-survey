...
function parseGraph(value: unknown): PublicGraph | null {
  ...
  const parseNode = (item: unknown): PublicGraphNode | null => {
    ...
    return { id: node.id, kind: node.kind as PublicGraphNode["kind"], name: node.name, evidenceState: node.evidenceState, sourceId: typeof node.sourceId === "string" ? node.sourceId : null, sourceTitle: typeof node.sourceTitle === "string" ? node.sourceTitle : null, sourceState: typeof node.sourceState === "string" ? node.sourceState as PublicGraphNode["sourceState"] : null };
  };
