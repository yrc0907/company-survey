...
function parseGraph(value: unknown): PublicGraph | null {
  ...
  const parseNode = (item: unknown): PublicGraphNode | null => {
    ...
    return { id: node.id, kind: node.kind as PublicGraphNode["kind"], name: node.name, evidenceState: node.evidenceState, sourceId: typeof node.sourceId === "string" ? node.sourceId : null, sourceTitle: typeof node.sourceTitle === "string" ? node.sourceTitle : null, sourceUrl: safeSourceUrl(node.sourceUrl), sourceCapturedAt: typeof node.sourceCapturedAt === "string" ? node.sourceCapturedAt : null, sourceContentHash: typeof node.sourceContentHash === "string" ? node.sourceContentHash : null, sourceState: typeof node.sourceState === "string" ? node.sourceState as PublicGraphNode["sourceState"] : null };
  };
  const parseEdge = (item: unknown): PublicGraphEdge | null => {
    ...
    return { id: edge.id, fromEntityId: edge.fromEntityId, toEntityId: edge.toEntityId, relation: edge.relation, evidenceState: edge.evidenceState, sourceId: typeof edge.sourceId === "string" ? edge.sourceId : null, sourceTitle: typeof edge.sourceTitle === "string" ? edge.sourceTitle : null, sourceUrl: safeSourceUrl(edge.sourceUrl), sourceCapturedAt: typeof edge.sourceCapturedAt === "string" ? edge.sourceCapturedAt : null, sourceContentHash: typeof edge.sourceContentHash === "string" ? edge.sourceContentHash : null, sourceState: typeof edge.sourceState === "string" ? edge.sourceState as PublicGraphEdge["sourceState"] : null };
  };
