...
import { FilePreview } from "@/components/platform/file-preview";
...
import { ProjectHistory } from "@/components/platform/project-history";
...
interface ProjectWorkspaceProps {
  ...
  /** 通知深链携带的评论 ID；公开内容加载后由评论面板定位。 */
  initialCommentId?: string | null;
  /** 通知深链可直接打开修改申请页。 */
  initialTab?: "changes" | null;
    ...
    <article className="knowledge-document">
      <header className="document-heading">
        ...
        <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4" aria-label="项目概览统计">
          ...
          <div className="rounded-lg border bg-muted/35 px-3 py-2.5"><span className="block text-[10px] uppercase tracking-wide text-muted-foreground">来源</span><strong className="mt-1 block font-mono text-sm tabular-nums">{project.sourceCount}</strong></div>
          <div className="rounded-lg border bg-muted/35 px-3 py-2.5"><span className="block text-[10px] uppercase tracking-wide text-muted-foreground">贡献者</span><strong className="mt-1 block font-mono text-sm tabular-nums">{project.contributorCount ?? project.contributors.length}</strong></div>
          <div className="rounded-lg border bg-muted/35 px-3 py-2.5"><span className="block text-[10px] uppercase tracking-wide text-muted-foreground">待审核</span><strong className="mt-1 block font-mono text-sm tabular-nums">{project.openMergeRequests}</strong></div>
        </div>
        {isOverview && overviewItems.length > 0 ? <div className="mt-6 border-t pt-5" aria-label="研究章节总览"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="m-0 text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">研究档案总览</p><p className="mb-0 mt-1 text-xs text-muted-foreground">点击章节进入完整正文；卡片只展示该章节真实首段与证据状态。</p></div><span className="shrink-0 rounded-full border px-2 py-1 text-[10px] text-muted-foreground">{project.sections.length} 个章节</span></div><div className="grid gap-2 sm:grid-cols-2">{overviewItems.map((item) => <button key={item.id} type="button" className="rounded-lg border bg-muted/20 p-3 text-left transition-colors hover:border-foreground/30 hover:bg-muted/45" onClick={() => item.nodeId && onSelectNode?.(item.nodeId)}><div className="flex items-center justify-between gap-2"><strong className="truncate text-sm">{item.heading}</strong><span className="shrink-0 text-[10px] text-muted-foreground">{evidenceCopy[item.state]}</span></div><p className="mb-0 mt-2 line-clamp-3 text-xs leading-5 text-muted-foreground">{item.paragraphs[0] ?? "该章节暂无公开正文。"}</p><span className="mt-2 block text-[10px] text-muted-foreground">{item.citations} 条引用 · 查看正文</span></button>)}</div></div> : null}
      </header>
...
/** 公开贡献者面板只读取项目返回的归因列表；作者入口沿用公开 /u/:username 路由。 */
function ProjectContributorsPanel({ project }: { project: SeedProject }) {
  return <section className="mx-auto w-full max-w-[860px] px-5 pb-20 pt-10 sm:px-14" aria-labelledby="project-contributors-title"><div className="border-b pb-6"><p className="mb-1 text-xs text-muted-foreground">公开项目</p><h1 id="project-contributors-title" className="m-0 text-2xl font-semibold">贡献者</h1><p className="mb-0 mt-2 text-sm text-muted-foreground">仅展示已经写入公开项目归因的用户。</p></div><div className="mt-6 grid gap-2">{project.contributors.length ? project.contributors.map((user) => <button key={user.id} type="button" className="flex items-center gap-3 rounded-lg border bg-background px-4 py-3 text-left transition-colors hover:bg-muted" onClick={() => window.location.assign(`/u/${encodeURIComponent(user.username)}`)}><UserAvatar name={user.displayName} size="md" /><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{user.displayName}</strong><small className="block truncate text-xs text-muted-foreground">@{user.username}</small></span><span className="text-xs text-muted-foreground">查看主页 <ChevronRight size={14} className="ml-1 inline" aria-hidden="true" /></span></button>) : <div className="grid place-items-center gap-2 py-20 text-center text-sm text-muted-foreground"><Users size={22} /><p className="m-0">暂无公开归因。</p></div>}</div></section>;
}

/** 问题面板不凭空生成问题；没有真实争议时显示可解释的空状态。 */
function ProjectIssuesPanel() {
  return <section className="mx-auto w-full max-w-[860px] px-5 pb-20 pt-10 sm:px-14" aria-labelledby="project-issues-title"><div className="border-b pb-6"><p className="mb-1 text-xs text-muted-foreground">公开项目</p><h1 id="project-issues-title" className="m-0 text-2xl font-semibold">问题</h1><p className="mb-0 mt-2 text-sm text-muted-foreground">问题由可追踪的评论、证据冲突或审核意见产生。</p></div><div className="mt-8 grid place-items-center gap-2 rounded-xl border border-dashed py-20 text-center text-sm text-muted-foreground"><CircleAlert size={22} /><p className="m-0">暂无待处理问题</p><span className="text-xs">当项目出现真实争议时，会在这里显示负责人、来源和处理状态。</span></div></section>;
}

/** 项目详情以稳定三栏承载文件、正文和 AI；未接入能力只展示明确门槛。 */
export function ProjectWorkspace({ project, onBack, onRequireLogin, initialCommentId = null, initialTab = null, initialChangeId = null }: ProjectWorkspaceProps) {
  const [activeNodeId, setActiveNodeId] = useState("doc-overview");
  const [treeCollapsed, setTreeCollapsed] = useState(false);
  const [activity, setActivity] = useState("");
  const [activeTab, setActiveTab] = useState<"content" | "graph" | "changes" | "activity" | "history" | "issues" | "contributors">("content");
  ...
  /** 下载公开主版本；浏览器只接收服务端生成的公开投影，不接触 OSS 原件。 */
  async function exportFile(format: "markdown" | "pdf"): Promise<void> {
    setExportState("loading");
    setExportError("");
    try {
      const isPdf = format === "pdf";
      const response = await fetch(`/api/platform/projects/${encodeURIComponent(project.id)}/export?format=${format}`, { headers: { accept: isPdf ? "application/pdf" : "text/markdown" }, cache: "no-store", credentials: "same-origin" });
      if (!response.ok) {
        const payload = await response.json().catch(() => ({})) as { error?: string };
        throw new Error(payload.error ?? `${isPdf ? "PDF" : "Markdown"} 导出失败`);
      }
      ...
      anchor.href = href;
      anchor.download = encodedFilename ? decodeURIComponent(encodedFilename) : `${project.slug}.${isPdf ? "pdf" : "md"}`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(href);
      setExportState("idle");
      setActivity(`${isPdf ? "PDF" : "Markdown"} 已导出。`);
    } catch (error) {
      setExportState("error");
      setExportError(error instanceof Error ? error.message : `${format === "pdf" ? "PDF" : "Markdown"} 导出失败`);
    }
  }
    ...
    <div className={treeCollapsed ? "workspace-layout tree-is-collapsed" : "workspace-layout"}>
      <header className="project-header">
        ...
        <div className="project-header__actions">
          ...
          <Button variant="ghost" size="sm" onClick={() => void exportFile("markdown")} disabled={exportState === "loading"} aria-busy={exportState === "loading"} title={exportState === "error" ? "导出失败，点击重试" : "下载公开 Markdown"}>{exportState === "loading" ? <Loader2 size={15} className="animate-spin" /> : <Download size={15} />}导出 Markdown</Button>
          <Button variant="ghost" size="sm" onClick={() => void exportFile("pdf")} disabled={exportState === "loading"} aria-busy={exportState === "loading"} title={exportState === "error" ? "导出失败，点击重试" : "下载公开 PDF"}>{exportState === "loading" ? <Loader2 size={15} className="animate-spin" /> : <Download size={15} />}导出 PDF</Button>
      ...
      <nav className="project-tabs" aria-label="项目导航"><button className={activeTab === "content" ? "is-active" : undefined} type="button" onClick={() => setActiveTab("content")}><BookOpen size={15} />内容</button><button className={activeTab === "graph" ? "is-active" : undefined} type="button" onClick={() => setActiveTab("graph")}><Network size={15} />关系图</button><button className={activeTab === "changes" ? "is-active" : undefined} type="button" onClick={() => setActiveTab("changes")}><GitPullRequest size={15} />修改申请 <span>{project.openMergeRequests}</span></button><button className={activeTab === "history" ? "is-active" : undefined} type="button" onClick={() => setActiveTab("history")}><GitMerge size={15} />版本</button><button className={activeTab === "issues" ? "is-active" : undefined} type="button" onClick={() => { setActiveTab("issues"); setActivity("问题面板将在有可追踪争议后显示。"); }}><CircleAlert size={15} />问题</button><button className={activeTab === "activity" ? "is-active" : undefined} type="button" onClick={() => setActiveTab("activity")}><History size={15} />历史</button><button className={activeTab === "contributors" ? "is-active" : undefined} type="button" onClick={() => { setActiveTab("contributors"); setActivity("贡献者列表将在首个真实合并后显示。"); }}><Users size={15} />贡献者</button></nav>
