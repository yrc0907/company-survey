...
const intentCopy: Record<LoginIntent, { title: string; description: string }> = {
  ...
  contribute: { title: "登录后提交贡献", description: "游客可以阅读公开内容；创建合并申请与永久署名需要登录。" },
};
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="login-dialog">
        ...
        {authEnabled ? <>
          ...
          <p className="login-status">登录后会返回你刚才的操作；AI 助手和写入能力仅对登录用户开放。</p>
        </> : <>
          <DialogTitle>{PUBLIC_AUTH_CLOSED_MESSAGE}</DialogTitle>
          <DialogDescription>当前不接受登录、注册、上传、创建项目或提交贡献申请。公开研究和搜索仍可使用，AI 助手仅对内测用户开放。</DialogDescription>
