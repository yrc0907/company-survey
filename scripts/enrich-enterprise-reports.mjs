...
async function enrichCompany(tx, [projectId, company, url, focus]) {
  ...
  await tx`INSERT INTO knowledge_commit (id, project_id, branch_id, parent_commit_id, author_user_id, message, ai_assisted, idempotency_key, idempotency_fingerprint, change_summary, created_at)
    VALUES (${commitId}, ${projectId}, ${project.branch_id}, ${project.head_commit_id ? String(project.head_commit_id) : null}, ${String(project.owner_user_id)}, ${`导入${company}官方资料并补齐独立研究章节`}, FALSE, ${`official-dossier:${projectId}:${DOSSIER_VERSION}`}, ${contentHash}, ${JSON.stringify({ source: url, evidenceState: "needs_verification", dossierVersion: DOSSIER_VERSION })}::jsonb, ${now})`;
